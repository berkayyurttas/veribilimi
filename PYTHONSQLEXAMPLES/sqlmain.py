import sqlite3
import os

def create_database():
    if os.path.exists("students.db"):
        os.remove("students.db")

    
    conn = sqlite3.connect("students.db")
    cursor = conn.cursor()
    return conn, cursor

def create_tables(cursor):
    cursor.execute('''
                   
       CREATE TABLE Students (
          id INTEGER PRIMARY KEY,
          name VARCHAR NOT NULL,
          age INTEGER,
          email VARCHAR UNIQUE,
          city VARCHAR)
        
    ''')

    cursor.execute('''
                   
       CREATE TABLE Courses (
            id INTEGER PRIMARY KEY,
            course_name VARCHAR NOT NULL,
            instructor TEXT,
            credits INTEGER)
        
    ''')

def insert_sample_data(cursor):
    
    students = [
        (1, 'Alice Johnson', 20, 'alice@gmail.com', 'New York'),
        (2, 'Bob Smith', 19, 'bob@gmail.com', 'Chicago'),
        (3, 'Carol White', 21, 'carol@gmail.com', 'Boston'),
        (4, 'David Brown', 20, 'david@gmail.com', 'New York'),
        (5, 'Emma Davis', 22, 'emma@gmail.com', 'Seattle')
    ]
    cursor.executemany("INSERT INTO Students VALUES (?,?,?,?,?)", students)
    
    cources = [
        (1, 'Python Programming', 'Dr.Anderson', 3),
        (2, "Web Development", 'Prof.Wilson', 4),
        (3, 'Data Science', 'Dr. Taylor', 3),
        (4, 'Mobile Apps', 'Prof.Garcia', 2)
    ]

    # Düzeltme: Tablo adı 'Courses' yapıldı ve 'VALUE' yerine 'VALUES' kullanıldı
    cursor.executemany("INSERT INTO Courses VALUES (?,?,?,?)", cources)
    print("Sample data inserted successfully")

def basic_sql_operations(cursor):
    #1)SELECT ALL
    print("---------Select All------")
    cursor.execute("SELECT * FROM Students")
    records = cursor.fetchall()
    for row in records:
        print(row[0], row[1], row[2], row[3])

    #2) SELECT Columns
    print("---------Select Column------")
    cursor.execute("SELECT name, age FROM Students")
    records = cursor.fetchall()
    print(records)

    #3) WHERE clause
    print("---------Where Age = 20-------")
    cursor.execute("SELECT * FROM Students WHERE age = 20")
    records = cursor.fetchall()
    for row in records:
        print(row)





    #4) WHERE with string
    print("---------Where city = New York -------")
    cursor.execute("SELECT * FROM Students WHERE city = 'New York'")
    records = cursor.fetchall()
    for row in records:
        print(row)

    #5) ORDER BY
    print("-------Order by age ----------")
    cursor.execute("SELECT * FROM Students ORDER BY age")
    records = cursor.fetchall()
    for row in records:
        print(row)
    
    #6) LIMIT
    print("---------Limit by 3 --------")
    cursor.execute("SELECT * FROM Students LIMIT 3")
    records = cursor.fetchall()
    for row in records:
        print(row)
    
def sql_update_delete_insert_operations(conn, cursor): 
    #1) INSERT
    print("\n---------Inserting Frank Miller-------")
    cursor.execute("INSERT INTO Students VALUES (6, 'Frank Miller', 23, 'frank@gmail.com', 'Miami')")
    conn.commit()

    #2) UPDATE
    cursor.execute("UPDATE Students SET age = 24 WHERE id = 6")
    conn.commit()

    #3)DELETE
    cursor.execute("DELETE FROM Students WHERE id = 6")
    conn.commit()

def aggregate_function(cursor):
    
    #1) Average
    print("--------Aggregate Function-----")
    cursor.execute("SELECT COUNT(*) FROM Students")
    result = cursor.fetchone()
    print(result[0])
 
     #2)COUNT
    print("--------Aggregate Function-----")
    cursor.execute("SELECT AVG(age) FROM Students")
    result = cursor.fetchone()
    print(result[0])
  
     #3)MAX - MIN
    print("--------Aggregate Function-----")
    cursor.execute("SELECT MAX(age), MIN(age) FROM Students")
    result = cursor.fetchone()
    max_age, min_age = result
    print(max_age)
    print(min_age)


     #4) GROUP BY
    print("--------Aggregate Function-----")
    cursor.execute("SELECT city, COUNT(*) FROM Students GROUP BY city")
    result = cursor.fetchall()
    print(result)

def answers(cursor):
    print("\n------ SINAV CEVAPLARI ------")
    
    # 1-Bütün kursların bilgilerini getirin
    print("\n1-Bütün kursların bilgilerini getirin")
    cursor.execute("SELECT * FROM Courses")
    print(cursor.fetchall())

    # 2- Sadece eğitmenlerin ismini ve ders ismi bilgilerini getiriniz
    # DÜZELTME: Sütun adları course_name ve instructor olarak düzeltildi.
    print("\n2- Sadece eğitmenlerin ismini ve ders ismi bilgilerini getiriniz")
    cursor.execute("SELECT course_name, instructor FROM Courses")
    print(cursor.fetchall())

    # 3-Sdece 21 yaşındaki öğrencileri getirin
    print("\n3-Sdece 21 yaşındaki öğrencileri getirin")
    cursor.execute("SELECT * FROM Students WHERE age = 21")
    print(cursor.fetchall())

    # 4- Sadece Chicago'da yaşayan öğrencileri getirin
    # DÜZELTME: FROM Students eklenmiştir.
    print("\n4- Sadece Chicago'da yaşayan öğrencileri getirin")
    cursor.execute("SELECT * FROM Students WHERE city = 'Chicago'")
    print(cursor.fetchall())

    # 5- Sadece 'Dr.Anderson' tarafından verilen dersleri getirin
    print("\n5- Sadece 'Dr.Anderson' tarafından verilen dersleri getirin")
    cursor.execute("SELECT * FROM Courses WHERE instructor = 'Dr.Anderson'")
    print(cursor.fetchall())

    # 6-Sadece ismi 'A' ile başlayan öğrencileri getirin
    print("\n6-Sadece ismi 'A' ile başlayan öğrencileri getirin (LIKE kullanıldı)")
    cursor.execute("SELECT * FROM Students WHERE name LIKE 'A%'")
    print(cursor.fetchall())

    # 7-Sdece 3 ve üzeri kredi olan dersleri getirin
    print("\n7-Sdece 3 ve üzeri kredi olan dersleri getirin")
    cursor.execute("SELECT * FROM Courses WHERE credits >= 3")
    print(cursor.fetchall())

    # 8-Öğrencileri alfabetik şekilde dizerek getiriniz (Soru 1 olarak görünüyor)
    print("\n8-Öğrencileri alfabetik şekilde dizerek getiriniz (ORDER BY)")
    cursor.execute("SELECT * FROM Students ORDER BY name")
    print(cursor.fetchall())

    # 9- 20 yaşından büyük öğrencileri ismine göre sıralayarak getiriniz. (Soru 2 olarak görünüyor)
    print("\n9- 20 yaşından büyük öğrencileri ismine göre sıralayarak getiriniz.")
    cursor.execute("SELECT name, age FROM Students WHERE age > 20 ORDER BY name")
    print(cursor.fetchall())

    # 10- Sdece 'New York' veya 'Chicago' da yaşayan öğrencileri getirin (Soru 3 olarak görünüyor)
    print("\n10- Sdece 'New York' veya 'Chicago' da yaşayan öğrencileri getirin (IN kullanıldı)")
    cursor.execute("SELECT name, city FROM Students WHERE city IN ('New York', 'Chicago')")
    print(cursor.fetchall())

    # 11-Sadece 'New Yorkta' yaşamayan öğrencileri getirin (Soru 4 olarak görünüyor)
    # DÜZELTME: Tablo adı Students yapıldı ve tırnak kapatıldı.
    print("\n11-Sadece 'New Yorkta' yaşamayan öğrencileri getirin (NOT EQUAL !=)")
    cursor.execute("SELECT name, city FROM Students WHERE city != 'New York'")
    print(cursor.fetchall())

def main():
    conn, cursor = create_database()

    try:
        create_tables(cursor)
        insert_sample_data(cursor)
        basic_sql_operations(cursor)
        sql_update_delete_insert_operations(conn, cursor)
        aggregate_function(cursor)
        answers(cursor)
        conn.commit()
    except sqlite3.Error as e:
        print(e)

    finally:
        conn.close()
    

if  __name__ == "__main__":

    main()

